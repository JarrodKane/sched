<!--
  +page.svelte — /accounts/[id]/lineups/[lineupId]
  Single lineup detail. Shows a capacity bar, editable notes, and the entry list
  with status-cycle buttons, move-up/down controls, and the AddActForm component.

  localEntries is a $state copy of data.entries that AddActForm mutates directly
  for optimistic inserts. A $effect re-syncs it whenever server data refreshes
  (i.e. after a move, status change, or other mutation).

  Svelte features:
    $state         — localEntries (local copy of data.entries), editingNotes, notesText,
                     movingId (disables move buttons while a swap is in flight)
    $effect        — re-syncs localEntries from data.entries after each server load
    $props()       — receives data (lineup, show, entries) and form (action result)
    use:enhance    — on status cycle, move, and notes forms; move does an optimistic
                     swap and rolls back on failure
    bind:localEntries — passes localEntries to AddActForm as a $bindable prop so it
                        can insert a temp row for instant visual feedback
-->

<script lang="ts">
	import { enhance } from '$app/forms';
	import AddActForm from '$lib/components/AddActForm.svelte';
	...
	const STATUS_STYLES: Record<string, string> = { booked: 'badge-success', to_contact: 'badge-warning', cancelled: 'badge-error badge-soft' };
...
{#if data.show.actsPerShow}
	<div class="mb-5">
		...
		<div class="w-full h-2 bg-base-300 rounded-full overflow-hidden">
			<div class="h-full rounded-full transition-all duration-300 {overCapacity ? 'bg-error' : atCapacity ? 'bg-success' : 'bg-primary'}"
				style="width: {Math.min(100, (activeCount / data.show.actsPerShow) * 100)}%"></div>
		</div>
	</div>
{/if}
...
<div class="flex flex-col gap-2 mb-5">
	...
	{#each localEntries as entry, i (entry.id)}
		...
		<div class="rounded-2xl bg-base-100 border border-base-200 overflow-hidden">
			...
			<div class="flex items-center gap-2 px-3 py-3 min-h-14">
				...
				>
					...
					<button type="submit" disabled={entry.id.startsWith('temp_')} title="Tap to advance status"
				...
				<!-- Download photo -->
				{#if entry.photoUrl}
					<button type="button" onclick={() => downloadPersonPhoto(entry.photoUrl!, entry.name)}
						class="btn btn-xs btn-soft btn-square shrink-0" title="Download {entry.name}'s photo">
				...
				<!-- Edit toggle -->
				<button type="button" aria-label={editingId === entry.id ? 'Close edit' : 'Edit'} disabled={entry.id.startsWith('temp_')}
				...
				<div class="border-t border-base-200 px-3 pb-4 pt-3 bg-base-200/30">
					...
					>
						...
						<div class="flex items-center justify-between gap-2">
							...
							<button type="submit" formaction="?/removeEntry" class="btn btn-sm btn-soft btn-error"
								onclick={(e) => { if (!confirm(`Remove ${entry.name}?`)) e.preventDefault(); }}>Remove</button>
						</div>
					</form>
				</div>
			{/if}
