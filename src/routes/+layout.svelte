...
{#if data.session && data.profile}
	<div class="navbar bg-base-100 border-b border-base-300 px-3 shadow-sm">
				...
				<ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-50 w-52 p-2 shadow-lg border border-base-300 mt-1">
					...
					<li class="border-t border-base-200 mt-1 pt-1">
						<form method="POST" action="/login?/logout">
							<button type="submit" class="w-full text-left text-error">Sign out</button>
						</form>
					</li>
				</ul>
			</div>
		...
		<div class="navbar-end gap-1">
			<span class="text-sm text-base-content/50 hidden sm:inline mr-1">{data.profile.name}</span>
			<button
				type="button"
				onclick={toggleTheme}
...
<main class="min-h-screen bg-base-200">
	<div class="mx-auto max-w-6xl px-3 sm:px-4 py-5 sm:py-8">
		{@render children()}
	</div>
</main>
